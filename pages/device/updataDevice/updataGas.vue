<template>
	<view class="content">
		<u-navbar :is-back="true" title="更新燃气泄漏报警设备" :title-width="300" title-color="#000000" :title-size="36" />
		<u-form :model="form" ref="uForm">
			<view class="info">
				<u-form-item label="接入Dtu" label-width="242" :label-style="{paddingLeft: '24rpx'}">
					<u-input v-if="form.Dtu" v-model="form.Dtu" input-align="right" placeholder="请输入内容" type="select"
						:select-open="pickerShow" @click="DtuShow = true" />
				</u-form-item>
				<u-form-item label="接入报警主机" label-width="242" :label-style="{paddingLeft: '24rpx'}">
					<u-input v-if="form.Alarm" v-model="form.Alarm" input-align="right" placeholder="请输入内容" type="select"
		...
		<u-select v-model="pickerShow" :list="gatewayList" @confirm="selectConfirm" />
		<u-select v-model="areaShow" :list="areaList" @confirm="areaConfirm" />
		<u-select v-model="DtuShow" :list="areaList" @confirm="areaConfirm" />
	</view>
</template>
		...
		methods: {
			...
			submit() {
				...
				this.$refs.uForm.validate(valid => {
					if (valid) {
						this.$u.api.updatedDviceGas(this.form).then(res => {
							this.$u.toast('更新成功！')
							setTimeout(() => {
								this.$u.route({

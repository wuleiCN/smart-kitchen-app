<template>
	<view class="content">
		<u-navbar :is-back="true" title="更新智能网关" :title-width="300" title-color="#000000" :title-size="36" />
		<u-form :model="form" ref="uForm">
			<view class="info">
				...
				<u-form-item label="端口号" label-width="242" :label-style="{paddingLeft: '24rpx'}" prop="PhoneNumber">
					<u-input v-model="form.PhoneNumber" input-align="right" placeholder="请输入端口号" />
				</u-form-item>
			</view>
			<view class="info">
		...
		methods: {
			...
			submit() {
				...
				this.$refs.uForm.validate(valid => {
					if (valid) {
						this.$u.api.updatedDviceGateway(this.form).then(res => {
							this.$u.toast('更新成功！')
							setTimeout(() => {
								this.$u.route({

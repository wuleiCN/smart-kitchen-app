<template>
	<view class="content">
		<u-navbar :is-back="true" title="设备施工" :title-width="300" />
		...
		<view class="section u-flex">
			<span class="line" />
			<span class="_title u-flex">施工清单</span>
		</view>
		<scroll-view class="product" show-scrollbar scroll-top="0" :scroll-y="true" :lower-threshold="5"
			...
			<view class="item u-border-bottom" v-for="(item, index) in list"
				@click="toDevicedistribute(item.ModelId,item.Installed,item.Count,item.Type)">
				...
				<view class="title-wrap">
					...
					<text>设备数量：{{item.Installed}}/{{item.Count}}</text>
				</view>
			</view>
		...
		<view class="dispatch">
			<u-button hover-class="none" @click="dispatchOrder">施工回单</u-button>
...
<script>
	export default {
		...
		methods: {
			...
			// 施工回单
			dispatchOrder() {
				this.$u.api.canSaleInstall({
					order: this.optionId
				}).then(res => {
					if (res.data.Can) this.dispatchShow = true
					else this.$u.toast('设备尚未施工完成！')
				})
			},
			// 派单
			dispatch() {
				this.$u.api.finishSaleInstall({
					id: this.optionId
				}).then(res => {
					if (res.success) {
						this.$u.toast('出库回单成功！')
			...
			// 设备施工
			toDevicedistribute(id, d, c, type) {
				if (d !== c) {
				...
				} else this.$u.toast('已出库,请选择未施工的设备！')
			},
			// 上拉加载
			toLowFun() {
